...
<style>
    ...
    .stFileUploader {{ border: 2px dashed #0066cc !important; border-radius: 10px; padding: 10px; background-color: {card_bg}; }}
    [data-testid="stMetricValue"] {{ font-size: 2.2rem !important; font-weight: 700 !important; color: #0066cc; }}
    .info-box {{ background-color: {box_bg}; padding: 15px; border-radius: 8px; border-left: 5px solid #0066cc; margin-bottom: 15px; color: {text_color}; }}
    .log-box {{ background-color: {card_bg}; padding: 10px; border-radius: 5px; margin-top: 5px; border: 1px solid rgba(0,102,204,0.3); }}
</style>
...
LANG_DICT = {
    "繁體中文": {
        ...
        "tech_title": "🗣️ 現場技術員與 ME 經驗回報日誌 (人因補充點)",
        "tech_input_lbl": "請輸入今日現場觀察 (例如：某機台有異音、漏氣...)",
        "tech_btn": "送出日誌",
        "tech_empty": "目前尚無現場回報日誌。"
    },
    "English": {
        ...
        "tech_title": "🗣️ Technician & ME Field Observation Log",
        "tech_input_lbl": "Enter field observations (e.g., unusual noise, micro-leaks...)",
        "tech_btn": "Submit Log",
        "tech_empty": "No field logs available yet."
    },
    "ภาษาไทย": {
        ...
        "tech_title": "🗣️ บันทึกการสังเกตการณ์หน้างานของช่างเทคนิคและ ME",
        "tech_input_lbl": "กรอกข้อมูลที่พบเจอหน้างาน (เช่น มีเสียงผิดปกติ, ลมรั่วซึม...)",
        "tech_btn": "บันทึกข้อมูล",
        "tech_empty": "ยังไม่มีข้อมูลบันทึกจากหน้างาน"
    }
}
...
if uploaded_file is not None:
    ...
    with tab1:
        ...
        with c_col2:
            if "อาการเสีย / ปัญหา" in df.columns:
                ...
                fig_r = px.bar(reason_counts.reset_index(), x="count", y="อาการเสีย / ปัญหา",
                               orientation="h", title="Top 5 核心故障原因分析", template=plotly_template)
                st.plotly_chart(fig_r, use_container_width=True)

        # 現場回報日誌（修正後的版本，移除了 _with）
        st.write("---")
        st.markdown(f"### {L['tech_title']}")
        with st.form("tech_log_form", clear_on_submit=True):
            user_log = st.text_input(L["tech_input_lbl"])
            submitted = st.form_submit_button(L["tech_btn"])
            if submitted and user_log:
                now_str = datetime.now().strftime("%Y-%m-%d %H:%M")
                st.session_state.tech_logs.insert(0, f"⏱️ [{now_str}] {user_log}")

        if st.session_state.tech_logs:
            for log in st.session_state.tech_logs:
                st.markdown(f'<div class="log-box">{log}</div>', unsafe_allow_html=True)
        else:
            st.info(L["tech_empty"])
    ...
    with tab2:
        ...
        if predict_data:
            ...
            st.download_button(L["btn_dl"], pred_df.to_csv(index=False).encode('utf-8-sig'),
                               "Part_Prediction_Report.csv", "text/csv")
        else:
            st.write("暫無資料。")
    ...
    with tab3:
        ...
        if not pm_df.empty:
            ...
            st.download_button(L["btn_dl"], pm_df.to_csv(index=False).encode('utf-8-sig'), "Master_PM_Schedule.csv",
                               "text/csv")
        else:
